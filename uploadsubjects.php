<?php
		...
		if (move_uploaded_file($_FILES["xls"]["tmp_name"], $target_file)) {
			...
			for($i=2;$i<=$arrayCount;$i++){
				...
				$dept=$allDataInSheet[$i]["C"];
				if(!empty($subcode)){
					mysql_query("SELECT * FROM subjects WHERE subcode='".$subcode."' AND dept_id='".$dept."'");
			...
			if($count==0)
			{
				...
				{					
					...
					$credits=$allDataInSheet[$i]["G"];
					mysql_query("INSERT INTO subjects VALUES (NULL,'$subname','$subcode',$dptid,$sem,$year,'$reg',$credits)"); 
